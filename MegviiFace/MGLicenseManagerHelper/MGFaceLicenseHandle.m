...
//因为pod库工程中的资源文件不能打包到main bundle，因此此处代码需要在源码的基础上修改，新增了一个MegviiFace.bundle文件，model文件放在bundle中
+ (NSString *)getModelFilePath {
    
    NSBundle *bundle = [NSBundle bundleWithPath:[[NSBundle bundleForClass:[MGFaceLicenseHandle class]] pathForResource:@"MegviiFace" ofType:@"bundle"]];
    NSString *modelPath = [bundle pathForResource:KMGFACEMODELNAME ofType:@""];
    return modelPath;
}
...
+ (NSDate *)getLicenseDate {
    ...
    NSString *modelPath = [self getModelFilePath];
